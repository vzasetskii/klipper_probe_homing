...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/probe_homing.py" "${KLIPPER_PATH}/klippy/extras/probe_homing.py"
    echo "[OK]"
}
...
add_updater()
{
    ...
    update_section=$(grep -c '\[update_manager[a-z ]* probe_homing\]' $MOONRAKER_CONFIG || true)
    if [ "$update_section" -eq 0 ]; then
        echo -e "\n[update_manager probe_homing]" >> "$MOONRAKER_CONFIG"
        echo "type: git_repo" >> "$MOONRAKER_CONFIG"
        echo "path: ${SRCDIR}" >> "$MOONRAKER_CONFIG"
        echo "origin: https://github.com/vzasetskii/klipper_probe_homing.git" >> "$MOONRAKER_CONFIG"
